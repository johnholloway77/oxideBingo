...
type HtmlToImage = {
  toBlob: (node: HTMLElement, options?: RenderOptions) => Promise<Blob | null>;
  toJpeg: (node: HTMLElement, options?: RenderOptions) => Promise<string>;
};
...
export function addImageCopy(buttonId: string, targetId: string): void {
  ...
  btn.addEventListener("click", async (): Promise<void> => {
    if (!htmlToImage) {
      htmlToImage =
        await import("https://unpkg.com/html-to-image@1.11.11/es/index.js");
    }
    ...
    const rectangle: DOMRect = target.getBoundingClientRect();
